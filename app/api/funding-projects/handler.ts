...
export async function handleGetFundingProjects(request: Request) {
  ...
  try {
    // Query SEM comparacao de enum - apenas busca e converte para texto
    ...
    // Filtrar em JS (nao no SQL para evitar problemas de enum)
...
function mapProjectStatus(s: string | null): string {
  if (!s) return "FUNDING"
